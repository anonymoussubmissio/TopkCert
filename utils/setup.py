...
def get_model(model_name,dataset_name,model_dir, ablation_size=19, ablation_type='column'):
    ...
    if not timm_pretrained: 
        ...
        checkpoint_name =model_name + '_{}_drs_{}_{}_{}_{}.pth'.format(dataset_name, ablation_size, dataset_name, ablation_type, dataset_name)
...
def get_data_loader(dataset_name,data_dir,model,batch_size=1,num_img=-1,train=False):
    ...
    if dataset_name in ['imagenette','imagenet']:
    ...
    elif dataset_name in ['cifar','cifar100','svhn','gtsrb','flowers102']:
        ...
        if dataset_name == 'cifar':
            ...
            split = 'train' if train else 'test'
            dataset_ = datasets.Flowers102(root=data_dir, split=split, download=True, transform=ds_transforms)
